...
Page({
  ...
  async load() { this.setData({ loading: true, error: '' }); try { const suffix = this.data.type ? `&type=${this.data.type}` : ''; this.setData({ posts: await api.get(`/posts?scope=school${suffix}`) }); } catch (error) { this.setData({ error: error.message }); } finally { this.setData({ loading: false }); } },
