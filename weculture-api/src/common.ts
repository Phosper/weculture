import { ArgumentsHost, Catch, ExceptionFilter, HttpException, HttpStatus, Injectable, NestInterceptor, CallHandler } from '@nestjs/common';
...
export class ApiErrorFilter implements ExceptionFilter {
    ...
    response.status(exception.getStatus()).json({ code: `HTTP_${exception.getStatus()}`, message: Array.isArray(message) ? message[0] : message, requestId: randomUUID() });
  }
}
