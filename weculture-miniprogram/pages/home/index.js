const { api } = require('../../services/api'); const { requireLogin } = require('../../utils/guard'); const { formatPost } = require('../../utils/format');
Page({
  ...
  async loadPosts() { this.setData({ loading: true, error: '' }); try { const q = this.data.q ? `&q=${encodeURIComponent(this.data.q)}` : ''; const posts = await api.get(`/posts?scope=platform${q}`); this.setData({ posts: posts.map(formatPost) }); } catch (error) { this.setData({ error: error.message }); } finally { this.setData({ loading: false }); } },
